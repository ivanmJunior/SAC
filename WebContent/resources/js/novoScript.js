...
function habilitaComponente(){
	if(document.querySelector('#inputFuncao') != null){
		...
		if(funcao.value == "NUTRICIONISTA"){
		        ...
		        for(i = 0; i < document.querySelector('#cbBoxFuncoes').options.length; i++){
					if(document.querySelector('#cbBoxFuncoes').options[i].text == funcao.value){
						$(document.querySelector('#cbBoxFuncoes')).prop("value",
								document.querySelector('#cbBoxFuncoes').options[i].value);
						break;
					}
				...
				$(document.querySelector('#lblModalidades')).prop("hidden", true);
...
function habilitaCamposFuncao(comboBoxFuncao){
	
	var funcao = comboBoxFuncao;
	
	if(funcao.options[funcao.value].text == "NUTRICIONISTA"){
	        $(document.querySelector('#validationCRN')).prop("disabled", false);
	        $(document.querySelector('#validationCRN')).prop("required", true);
	        $(document.querySelector('#cbBoxModalidades')).prop("hidden", true);
			$(document.querySelector('#btnAdd')).prop("hidden", true);
		...
		}else if(funcao.options[funcao.value].text == "INSTRUTOR"){
			$(document.querySelector('#cbBoxModalidades')).prop("hidden", false);
			$(document.querySelector('#btnAdd')).prop("hidden", false);
			$(document.querySelector('#btnRemove')).prop("hidden", false);
			$(document.querySelector('#btnAddAll')).prop("hidden", false);
			...
			$(document.querySelector('#lblEspecialidades')).prop("hidden", false)
			$(document.querySelector('#validationCRN')).prop("disabled", true);
	        $(document.querySelector('#validationCRN')).prop("required", false);
...
function checkedRadio(obj){
	...
	if(!document.querySelector('#inlineRadio1').checked && !document.querySelector('#inlineRadio2').checked){
		document.querySelector('#pMensagem').innerText = 'Selecione o Tipo de Usuario: Cliente/Funcionario!';
		$(document.querySelector('#cbBoxNivelAcesso')).prop("value", "");
		$('#mensagemModalUsuarios').modal();
	}else{
		if(obj.name == "umCliente"){
			$(document.querySelector('#cbBoxNivelAcesso')).prop("value", 1);
			$(document.querySelector('#lblNivelAcesso')).prop("hidden", true);
			$(document.querySelector('#cbBoxNivelAcesso')).prop("hidden", true);
			$(document.querySelector('#inlineRadio2')).prop("checked", false);
		
		}else if(obj.name == "umFuncionario"){
			$(document.querySelector('#cbBoxNivelAcesso')).prop("value", "");
			$(document.querySelector('#lblNivelAcesso')).prop("hidden", false);
			$(document.querySelector('#cbBoxNivelAcesso')).prop("hidden", false);
			$(document.querySelector('#inlineRadio1')).prop("checked", false);
		}
	}
